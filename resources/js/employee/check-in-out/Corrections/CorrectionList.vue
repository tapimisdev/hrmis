    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
      <div class="modal-content modern-modal">
            ...
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <tr v-for="(request, index) in requests" :key="index">
                  ...
                  <td>{{ formatTime(request.break_in) }}</td>
                  <td>{{ formatTime(request.time_out) }}</td>
                  <td>{{ formatTime(request.overtime_in) }}</td>
                  <td>{{ formatTime(request.overtime_out) }}</td>
                  <td>
                    <span
                      :class="{
        ...
        <div class="modal-footer">
          <button class="btn py-2 px-4 btn-danger" data-bs-dismiss="modal">
            Close
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    formatTime(datetime) {
      return datetime ? new Date(datetime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "-";
    },
    storageUrl(path) {
