      ...
      <div class="card shadow-sm w-100 dtr-card">
        ...
        <div class="card-body">
          ...
          <div v-if="topRows.length" class="top-table-wrap mb-3">
            <table class="table table-sm table-bordered align-middle top-info-table">
              <tbody>
                <tr v-for="(pair, idx) in topRows" :key="idx">
                  <template v-if="pair.length === 1">
                    <th class="top-th">{{ pair[0].label }}</th>
                    <td class="top-td" colspan="3">{{ pair[0].value }}</td>
                  </template>

                  <template v-else>
                    <th class="top-th">{{ pair[0].label }}</th>
                    <td class="top-td">{{ pair[0].value }}</td>
                    <th class="top-th">{{ pair[1].label }}</th>
                    <td class="top-td">{{ pair[1].value }}</td>
                  </template>
                </tr>
              </tbody>
            </table>
          ...
          <div class="row g-3">
            ...
            <div class="col-12 col-lg-6">
              <div class="table-responsive">
                <table class="table table-sm table-bordered align-middle text-center dtr-table">
                  <thead class="table-secondary">
                    <tr>
                      <th rowspan="2" class="text-start" style="width: 44px;">Day</th>
                      <th colspan="2">Morning</th>
                      <th colspan="2">Afternoon</th>
                      <th colspan="2">Overtime</th>
                      <th rowspan="2" style="width: 74px;">Total</th>
                      <th rowspan="2" style="width: 180px;">Remarks</th>
                    </tr>
                    <tr>
                      <th>In</th><th>Out</th>
                      <th>In</th><th>Out</th>
                  ...
                  <tbody>
                    <tr v-for="row in rowsFirstHalf" :key="'l-' + row.dayNo">
                      <td class="text-start fw-semibold">{{ row.dayNo }}</td>

                      <td :class="cellClass(row,'morning_in')">{{ row.morning_in }}</td>
                      <td :class="cellClass(row,'morning_out')">{{ row.morning_out }}</td>

                      <td :class="cellClass(row,'afternoon_in')">{{ row.afternoon_in }}</td>
                      <td :class="cellClass(row,'afternoon_out')">{{ row.afternoon_out }}</td>

                      <td :class="cellClass(row,'overtime_in')">{{ row.overtime_in }}</td>
                      <td :class="cellClass(row,'overtime_out')">{{ row.overtime_out }}</td>

                      <td class="fw-semibold">{{ row.daily_total }}</td>

                      <td class="text-start">
                        <div class="remarks-cell">
                          <span
                            v-for="(rk, i) in filteredRemarks(row.remarks)"
                            :key="i"
                            class=""
                            :class="remarkBadgeClass(rk)"
                          >
                            {{ rk }}
                          </span>
                          <span v-if="filteredRemarks(row.remarks).length === 0" class="">
                            —
                          </span>
                        </div>
                      </td>
            ...
            <div class="col-12 col-lg-6">
              <div class="table-responsive">
                <table class="table table-sm table-bordered align-middle text-center dtr-table">
                  ...
                  <tbody>
                    <tr v-for="row in rowsSecondHalf" :key="'r-' + row.dayNo">
                      <td class="text-start fw-semibold">{{ row.dayNo }}</td>
                      ...
                      <td class="text-start">
                        <div class="remarks-cell">
                          ...
                          <span v-if="filteredRemarks(row.remarks).length === 0" class="text-body-secondary small">
                            —
                          </span>
                        </div>
                      </td>
          ...
          <div class="mt-3 small text-center fst-italic text-body-secondary">
            I hereby certify that the above records are true and correct.
          </div>

          <div class="d-flex justify-content-center mt-4">
            <div class="text-center" style="min-width: 220px;">
              <div class="border-top pt-1 small fw-semibold">EMPLOYEE'S SIGNATURE</div>
            </div>
          </div>
        </div>
      </div>
...
const props = defineProps({
  payload: {
    type: Object,
    default: () => ({ computedData: [], summary: [], payroll_value: {}, information: {} }),
  },
...
function hasRemarkExact(remarks, keyword) {
  ...
  return remarks.some(
    (r) => String(r).trim().toLowerCase() === String(keyword).trim().toLowerCase()
  )
}
...
const monthRows = computed(() => {
  ...
  for (let dayNo = 1; dayNo <= totalDays; dayNo++) {
    ...
    const overtimePair = chosen?.overtime ? String(chosen.overtime).split(' to ') : []
...
function cellClass(row, fieldKey) {
  ...
  if (hasRemarkExact(row.remarks, 'consider absent') || Number(row.late_undertime) > 0) {
    return 'text-danger text-decoration-underline fw-semibold'
  }

  return 'text-body text-decoration-underline'
}
</script>
