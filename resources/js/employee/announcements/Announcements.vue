    ...
    <div class="d-flex justify-content-end">
      <nav aria-label="Announcements Pagination" class="mt-4">
        <ul class="pagination justify-content-center">
          <!-- Previous Button -->
          <li :class="['page-item', { disabled: !announcements.prev_page_url }]">
            <button 
              class="page-link" 
          ...
          <li
            ...
            :key="page"
            :class="['page-item', { active: page === announcements.current_page }]"
          >
            <button class="page-link" @click="goToPage(page)">{{ page }}</button>
          </li>

          <!-- Next Button -->
          <li :class="['page-item', { disabled: !announcements.next_page_url }]">
            <button 
              class="page-link" 
              @click="goToPage(announcements.current_page + 1)" 
...
export default {
  ...
  computed: {
    totalPages() {
      return Array.from({ length: this.announcements.last_page }, (_, i) => i + 1);
    },
  },
  mounted() {
    ...
    this.fetchAnnouncements(`/employee/announcements?page=${page}${search ? `&search=${search}` : ''}`);
  },
  methods: {
    ...
    goToPage(page) {
      if (page >= 1 && page <= this.announcements.last_page) {
        const url = `/employee/announcements?page=${page}${this.searchQuery ? `&search=${this.searchQuery}` : ''}`;
    ...
    performSearch() {
      ...
      const url = `/employee/announcements?page=1${this.searchQuery ? `&search=${this.searchQuery}` : ''}`;
      this.fetchAnnouncements(url);
    },
    clearSearch() {
      this.searchQuery = '';
