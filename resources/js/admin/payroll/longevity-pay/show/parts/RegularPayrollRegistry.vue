<template>
  ...
  >
    ...
    <template #title>PAYROLL OF LONGEVITY PAY FOR THE MONTH OF {{ month }}</template>
...
export default {
  ...
  methods: {
    ...
    async adjustRow(emp) {
      ...
      try {
        await axios.post(
          `/api/payroll/longevity-pay/items/${this.payroll_no}/${emp.id}`,
