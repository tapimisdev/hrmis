...
export default {
    ...
    data() {
        return {
            ...
            steps: [
                {
                    label: "Create Longevity Payroll",
    ...
    methods: {
        ...
        async validateAndGetReview() {
            ...
            this.errors = {};
            try {
                const res = await axios.post("/api/payroll/longevity-pay/check-employees", this.form, {
                    headers: {
                ...
                this.nextDisabled = this.form.employees.eligible.length == 0;
                return true;
            } catch (error) {
        ...
        async submitForm() {
            ...
            try {
                const selectedEmployees =
                    this.form.employees?.eligible?.filter((employee) => Boolean(employee.selected)) || [];
                ...
                const res = await axios.post("/api/payroll/longevity-pay/generate", this.form, {
                    headers: {
                        Accept: "application/json",
                        Authorization: `Bearer ${this.token}`,
                    },
                ...
                window.location.href = `/admin/payroll/longevity-pay/${payroll_no}?batch_id=${batch_id}`;
                return true;
            } catch (error) {
                if (error.response?.status === 422) {
                    this.errors = error.response.data.errors;
