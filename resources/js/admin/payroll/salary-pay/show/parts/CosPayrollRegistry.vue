<template>
    ...
    >
        ...
        <template #notice>
            <div
                v-if="hasUndeductedReferences"
                class="undeducted-alert"
                role="alert"
            >
                <i class="fa-solid fa-triangle-exclamation"></i>
                <span>
                    This payroll has deduction reference amounts that are not
                    yet deducted.
                </span>
            </div>
        </template>
        ...
        <template #filters>
            <div class="payroll-filter-bar">
                ...
                <div class="payroll-filter-meta">
                    Showing {{ filteredEmployeeCount }} of
                    {{ totalEmployeeCount }}
                </div>
            </div>
        </template>
            ...
            <table class="excel-table">
                <thead>
                    <tr class="header-labels">
                        ...
                        <th class="p-4 earning" style="width: 100px">
                            Total Salary
                        </th>

                        <th class="p-4 deduction" style="width: 100px">
                            EWT <br />(2%)
                        </th>
                        <th class="p-4 deduction" style="width: 100px">
                            Percentage Tax <br />(3%)
                ...
                >
                    ...
                    >
                        ...
                        <td class="text-center number-cell">
                            {{ formatNumber(emp.monthly_rate) }}
                        </td>
                        <td class="text-center number-cell">
                            {{ formatNumber(emp.salary_earned) }}
                        </td>

                        <!-- AUT -->
                        <td
                            class="text-center number-cell deduction breakdown-cell"
                        >
                            <div class="breakdown-total">
                                {{ formatNumber(emp.aut) }}
                            </div>
                            
                            <div
                                v-if="hasDeductionBreakdown(emp, 'aut')"
                                class="deduction-breakdown"
                            >
                                <div
                                    v-for="(
                                        item, itemIndex
                                    ) in deductionBreakdown(emp, 'aut')"
                                    :key="`${emp.employee_no}-aut-${itemIndex}`"
                                    class="deduction-breakdown-row"
                                >
                                    <div class="deduction-breakdown-main">
                                        <span
                                            class="deduction-breakdown-payroll"
                                        >
                                            {{ deductionBreakdownLabel(item) }}
                                        </span>
                                        <span
                                            class="deduction-breakdown-amount"
                                        >
                                            {{ formatNumber(item.amount) }}
                                        </span>
                                    </div>
                                    <span
                                        v-if="item.period_covered"
                                        class="deduction-breakdown-period"
                                    >
                                        {{ item.period_covered }}
                                    </span>
                                </div>
                        ...
                        <td class="text-center number-cell">
                            {{ formatNumber(emp.holiday) }}
                        </td>

                        <td class="text-center number-cell earning">
                            {{ formatNumber(emp.total_salary) }}
                        </td>
                        ...
                        >
                            <div class="breakdown-total">
                                {{ formatNumber(emp.ewt_2) }}
                            </div>
                            <div
                                v-if="hasDeductionBreakdown(emp, 'ewt_2')"
                                class="deduction-breakdown"
                            >
                                <div
                                    v-for="(
                                        item, itemIndex
                                    ) in deductionBreakdown(emp, 'ewt_2')"
                        ...
                        >
                            <div class="breakdown-total">
                                {{ formatNumber(emp.percentage_tax_3) }}
                            </div>
                            <div
                                v-if="
                        ...
                        >
                            <div class="breakdown-total">
                                {{ formatNumber(emp.tax_ewt_5) }}
                            </div>
                            <div
                                v-if="hasDeductionBreakdown(emp, 'tax_ewt_5')"
                                class="deduction-breakdown"
                            >
                                <div
                                    v-for="(
                                        item, itemIndex
                                    ) in deductionBreakdown(emp, 'tax_ewt_5')"
                        ...
                        >
                            <div class="breakdown-total">
                                {{ formatNumber(emp.w_tax) }}
                            </div>
                            <div
                                v-if="hasDeductionBreakdown(emp, 'w_tax')"
                                class="deduction-breakdown"
                            >
                                <div
                                    v-for="(
                                        item, itemIndex
                                    ) in deductionBreakdown(emp, 'w_tax')"
                        ...
                        >
                            <div class="breakdown-total">
                                {{ formatNumber(emp.hmo) }}
                            </div>
                            <div
                                v-if="hasDeductionBreakdown(emp, 'hmo')"
                                class="deduction-breakdown"
                            >
                                <div
                                    v-for="(
                                        item, itemIndex
                                    ) in deductionBreakdown(emp, 'hmo')"
                        ...
                        <td class="text-center">
                            <input
                                ...
                                class="text-center bg-body-secondary form-control"
                                style="width: 100px"
                            />
                        </td>
                        ...
                        <td class="text-center">
                            <textarea
                                style="width: 250px"
                                class="text-center bg-body-secondary form-control my-3"
                                v-model="emp.remarks"
                                @change="adjustRow(emp)"
                            ></textarea>
                        </td>
                    ...
                    <tr class="project-total text-center">
                        <td colspan="2" class="text-end">
                            <strong>Total ({{ project.name }})</strong>
                        </td>

                        <td class="number-cell">
                            {{
                                formatNumber(
                                    projectTotals(project, "monthly_rate"),
                                )
                            }}
                        </td>
                        <td class="number-cell">
                            {{
                                formatNumber(
                                    projectTotals(project, "salary_earned"),
                                )
                            }}
                        </td>

                        <td class="number-cell deduction">
                            {{ formatNumber(projectTotals(project, "aut")) }}
                        </td>
                        <td class="number-cell">
                            {{
                                formatNumber(projectTotals(project, "overtime"))
                            }}
                        </td>
                        <td class="number-cell">
                            {{
                                formatNumber(projectTotals(project, "holiday"))
                            }}
                        </td>
                        <td class="number-cell earning">
                            {{
                                formatNumber(
                                    projectTotals(project, "total_salary"),
                                )
                            }}
                        </td>

                        <td class="number-cell deduction">
                            {{ formatNumber(projectTotals(project, "ewt_2")) }}
                        </td>
                        <td class="number-cell deduction">
                            {{
                                formatNumber(
                                    projectTotals(project, "percentage_tax_3"),
                                )
                            }}
                        </td>
                        <td class="number-cell deduction">
                            {{
                                formatNumber(
                                    projectTotals(project, "tax_ewt_5"),
                                )
                            }}
                        </td>
                        <td class="number-cell deduction">
                            {{ formatNumber(projectTotals(project, "w_tax")) }}
                        </td>
                        <td class="number-cell deduction">
                            {{ formatNumber(projectTotals(project, "hmo")) }}
                        </td>

                        <td class="number-cell">
                            {{
                                formatNumber(
                                    projectTotals(project, "adjustment"),
                                )
                            }}
                        </td>
                        <td class="number-cell net-salary">
                            <strong>{{
                                formatNumber(
                                    projectTotals(project, "net_salary"),
                                )
                            }}</strong>
                        </td>
                ...
                <tfoot>
                    <tr class="grand-total text-center">
                        ...
                        <td class="number-cell">
                            {{ formatNumber(grandTotals("salary_earned")) }}
                        </td>

                        <td class="number-cell deduction">
                            {{ formatNumber(grandTotals("aut")) }}
                        </td>
                        <td class="number-cell">
                            {{ formatNumber(grandTotals("overtime")) }}
                        </td>
                        <td class="number-cell">
                            {{ formatNumber(grandTotals("holiday")) }}
                        </td>
                        <td class="number-cell earning">
                            {{ formatNumber(grandTotals("total_salary")) }}
                        </td>

                        <td class="number-cell deduction">
                            {{ formatNumber(grandTotals("ewt_2")) }}
                        </td>
                        <td class="number-cell deduction">
                            {{ formatNumber(grandTotals("percentage_tax_3")) }}
                        </td>
                        <td class="number-cell deduction">
                            {{ formatNumber(grandTotals("tax_ewt_5")) }}
                        </td>
                        <td class="number-cell deduction">
                            {{ formatNumber(grandTotals("w_tax")) }}
                        </td>
                        <td class="number-cell deduction">
                            {{ formatNumber(grandTotals("hmo")) }}
                        </td>
                        ...
                        <td class="number-cell net-salary">
                            <strong>{{
                                formatNumber(grandTotals("net_salary"))
                            }}</strong>
                        </td>
    ...
    computed: {
        totalEmployeeCount() {
            return this.projects.reduce(
                (count, project) => count + project.employees.length,
                0,
            );
        },
        ...
        hasUndeductedReferences() {
            return this.filteredProjects.some((project) =>
                project.employees.some(
                    (emp) => this.deferredDeductionReference(emp).length > 0,
                ),
            );
        },
        projectOptions() {
            ...
            return this.projects
                ...
                .map((project) => {
                    const employees = project.employees.filter((emp) => {
                        ...
                        return (
                            matchesSearch && matchesPosition && matchesRemarks
                        );
                    });
    ...
    methods: {
        ...
        deferredDeductionReference(emp) {
            return Array.isArray(emp.deferred_deduction_reference)
                ? emp.deferred_deduction_reference.filter(
                      (item) => Number(item.amount) > 0,
                  )
                : [];
        },
        deferredDeductionReferenceAmount(emp, key) {
            const item = this.deferredDeductionReference(emp).find(
        ...
        projectDeferredReferenceTotals(project, key) {
            return project.employees.reduce(
                (total, emp) =>
                    total + this.deferredDeductionReferenceAmount(emp, key),
                0,
            );
        },
        ...
        grandDeferredReferenceTotals(key) {
            return this.filteredProjects.reduce(
                (total, project) =>
                    total + this.projectDeferredReferenceTotals(project, key),
                0,
            );
        },
        async adjustRow(emp) {
