                    ...
                    <tbody v-else>
                        ...
                        >
                            ...
                            <template v-if="getStatusBadge(log)">
                                <td colspan="8" class="status-cell">
                                    <div
                                        :class="[
                                            'status-badge',
                                            getStatusBadge(log).class,
                                        ]"
                                    >
                                        <i
                                            :class="getStatusBadge(log).icon"
                                        ></i>
                                        <span>{{
                                            getStatusBadge(log).text
                                        }}</span>
                                        <span
                                            v-if="getStatusBadge(log).extra"
                                            class="badge-extra"
                                        >
                            ...
                            <td class="action-cell">
                                <div
                                    class="d-flex justify-content-end gap-3 pe-2"
                                >
                                    <button
                                        ...
                                        @click="
                                            downloadDAR(log.user_id, log.accomplishments)
                                        "
                                    >
                                        <i
...
export default {
    ...
    methods: {
        ...
        hasOvertimeRemark(remarks) {
            return (
                this.hasRemark(remarks, "overtime") ||
                this.hasRemark(remarks, "pending overtime")
            );
        },
        getRowClass(remarks) {
            ...
            if (this.hasRemark(remarks, "special order"))
                return "row-special-order";
        ...
        getRemarkClass(remark) {
            ...
            if (remarkLower === "late" || remarkLower === "undertime")
            ...
            if (remarkLower.includes("special order"))
                return "status-special-order";
         ...
         downloadDAR(user_id, path) {
            axios
                .get("/admin/timekeeping/accomplishment-report", {
                    params: { user_id, path },
                })
                .then((response) => {
        ...
        getActions(remarks) {
            ...
            /* -------- SPECIAL ORDER -------- */
            if (
                has("special order-morning") ||
                has("special order-afternoon")
            ) {
                return buildAdjustmentAndCancel(
            ...
            if (
                isLeaveType &&
                (has("leave-morning") || has("leave-afternoon"))
            ) {
