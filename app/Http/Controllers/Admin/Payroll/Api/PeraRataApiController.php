...
use App\Http\Requests\Admin\PeraRata\CreateRequest;
...
class PeraRataApiController extends Controller
{
    ...
    public function getAdjustments(Request $request)
    {
        ...
        $holidays = $this->payroll_service->getHolidays($validated);
        $suspensions = $this->payroll_service->getSuspensions($validated);
    ...
    {
        $employees = DB::table('payroll_pera_rata_employee as pse')
            ->where('pse.payroll_pera_rata_id', $payroll_id)
            ->leftJoinSub(
