...
class PeraRataItemController extends Controller
{
    public function update($payroll_no, $payroll_emp_id, Request $request)
    {
        ...
        try {
            ...
            $table = 'payroll_pera_rata_employee';

            $item = DB::table($table)->where('id', $payroll_emp_id)->first();

            if (!$item) {
                abort(404, 'Pera Rata pay record not found.');
            }
            ...
            return response([
                'fetch_data' => true,
                'data' => $updated_item,
                'message' => 'Pera Rata pay item updated successfully',
                'status' => 'success'
            ]);
            ...
            return response([
                ...
                'status' => 'update Pera Rata pay item failed'
            ], 500);
        }
    }
}
