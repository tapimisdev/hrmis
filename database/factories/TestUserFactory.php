...
class TestUserFactory extends Factory
{
    ...
    public function withEmployeeData(): static
    {
        return $this->afterCreating(function (User $user) {
            $employeeNo = $this->faker->unique()->bothify('emp###');

            DB::table('employee_information')->insert([
                'user_id'        => $user->id,
                'employee_no'    => $employeeNo,
                'biometrics_id'  => $this->faker->randomNumber(5),
                'account_status' => 'active',
                'date_hired'     => now()->format('Y-m-d'),
                'salary_method'  => $this->faker->randomElement(['cash', 'bank']),
                'created_at'     => now(),
                'updated_at'     => now(),
            ]);
            ...
            DB::table('employee_organization')->insert([
                ...
                'employment_type_id' => $this->faker->numberBetween(1, 2),
            ...
            DB::table('employee_shift_work_schedule')->insert([
                ...
                'work_schedule_id' => $this->faker->numberBetween(1, 2),
                'effectivity_date' => now(),
            ]);
